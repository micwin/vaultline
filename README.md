...
2. `go run ./cmd/vaultline --addr 127.0.0.1:8428 store init project-a ./stores/project-a` creates and registers an additional store, prints a generated unseal key once, stores that key in the registry config, and leaves the new store unsealed. If you omit the path (`store init project-a`), Vaultline creates the store next to the default store under the same parent directory. Use `store add` instead when the target store already exists.
