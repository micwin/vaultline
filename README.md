...
8. Expose an additional remote listener with `go run ./cmd/vaultline --addr 127.0.0.1:8428 daemon bind 0.0.0.0:8384`, then explicitly allow clients with `go run ./cmd/vaultline --addr 127.0.0.1:8428 daemon allow 0.0.0.0:8384 192.168.3.0/24`. `daemon list-binds`, `daemon list-allows`, `daemon unallow`, and `daemon unbind` persist their changes in the daemon config so they survive restarts. Loopback remains implicitly available at all times.
9. Install shell completion with `source <(vaultline completion bash)` or `source <(vaultline completion zsh)`. Completion is dynamic for configured stores, binds, allow rules, and dotted secret prefixes.
10. Import Bitwarden data with `vaultline import bitwarden --all --store default --prefix bitwarden` (or `--item <name-or-id>`). This uses the installed `bw` CLI and the current unlocked `BW_SESSION`, and supports `--dry-run`, `--add-missing-keys`, and `--overwrite-existing-keys`. Prefix, folder, item, and field names are only normalized lightly (lowercase, whitespace/`_` removal, `/`/`:`/`,`/`!`/`(` / `)` → `.`, repeated dots collapsed). `-` is preserved because Vaultline keys allow it; any other invalid generated keys are reported and skipped so the source data can be corrected explicitly.
