...
- **API + CLI**: REST speaks JSON over HTTP, while the CLI wraps it with ergonomic commands (`vaultline secret put --name app.api-key --stdin`, `vaultline secret get --name ... --out file`, `vaultline secrets list`, `vaultline secrets set --name ...`, `vaultline daemon-stop`). The CLI enforces `localhost` sockets to avoid remote hops.
...
2. `go run ./cmd/vaultline --addr 127.0.0.1:8428 secret put --name api-key --value "$(openssl rand -hex 16)"`. When running in an interactive terminal you can omit `--value/--file/--stdin` and the CLI will prompt with masked input; on non-interactive shells it requires one of the explicit flags. You may also skip `--name` and pass the key as a positional argument (`vaultline secret put api-key --value ...`).
...
4. Inspect stored keys with `go run ./cmd/vaultline --addr 127.0.0.1:8428 secrets list` for a tabular view (add `--output json` for machine-readable output). You can use `vaultline secrets set/get/delete` as aliases for the `secret` command group.
