...
2. `go run ./cmd/vaultline --addr 127.0.0.1:8428 secret put --name api-key --value "$(openssl rand -hex 16)"`. If `--value/--file/--stdin` are omitted, the CLI masks an interactive prompt so you can type passwords safely.
