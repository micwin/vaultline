...
printf "super-secret" | "${VAULTLINE_CLI[@]}" --addr "${ADDR}" secret set local:smoke.token --stdin >/dev/null
"${VAULTLINE_CLI[@]}" --addr "${ADDR}" secret get local:smoke.token --out "${TMP_VALUE}" >/dev/null
